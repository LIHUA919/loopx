...
WIDESEARCH_VERIFICATION_HINT = (
    "VERIFICATION DISCIPLINE (required, budget-aware): "
    "1. Keep the total verification budget under 50 minutes and never let "
    "verification prevent writing a COMPLETE table; if the budget is nearly used, "
    "stop verifying and write the best answer you have. "
    "2. Enumerate the case's exact required columns and create research plus "
    "independent-verification Todos per column group, recording the authoritative "
    "source URL as evidence. "
    "3. For list or ranking-style cells, treat the official/primary publisher as the "
    "single authoritative reference. "
    "4. For detail cells (dates, deadlines, prices, fees, quantities, codes, names, "
    "addresses), require the primary/official source. When the column asks for a "
    "STANDARD value (standard deadline, standard fee), take only that standard value: "
    "explicitly reject early, priority, rolling, or spring rounds, and reject "
    "waived or promotional amounts. "
    "5. Cross-check with a second source ONLY when it is another independent primary "
    "source (e.g. a second official page); never use a third-party aggregator as a "
    "confirming source. If sources conflict, trust the primary. "
    "6. NEVER leave a cell empty: if a value cannot be confirmed within budget, write "
    "your best-effort value from the primary source rather than a blank. "
    "7. After composing the table, run ONE bounded review pass (at most 5 minutes) "
    "that re-reads each detail cell against its primary-source evidence and fixes "
    "only clear errors. "
    "8. Do not add speculative rows or cells. Once the table is complete, write the "
    "final answer, mark the LoopX goal complete, and stop."
)
