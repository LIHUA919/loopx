...
def _cursor_path(project: Path, route_key: str) -> Path:
    return project / ".loopx" / "inbox" / ".turn-start" / f"{route_key}.json"
...
def _route_receipt(
    *,
    ...
    cursor_path = _cursor_path(Path(config["project"]), route_key)
    try:
